...
class Sql:
	...
	def hidden_remover(self):
		...
		for self.file in self.files:
			...
			if str(df.columns[3]).count('Unnamed') > 0:
	...
	def sql_uploader(self):
		#lecteur de code dans le fichier text doit remplir un dict et non plus un array avec le code  lié à sa description du yalm (ankle, arm etc...)
		# 
		...
		for i in range(len(self.df.index)):
			for n in range(1, len(self.df.columns)):
				...
				try:
					...
					print(self.df)
					print(str(self.df.iloc[i][self.df.columns[1]]))
				...
				for result in range(len(myresult)):
					try:
						...
						print('nom colonne est : ' + str(self.names[index_namelist]))
						print('args est : ' + str(args))
						print('case finale est : ' + str(self.df.iloc[i][self.df.columns[n]]*multiplier))
						print('code est : ' + str(self.df.iloc[i][self.df.columns[1]]))
						print('liste id final: ' + str(myresult))
						print('id final : ' + str(myresult[result][0]))
						print('df est : ' + str(self.df))
						print(self.identifiants)
						print('------------------------------------------------------------------------------------')
